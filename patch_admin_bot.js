...
function run() {
    ...
    // Replace currentAdminToken definition
    code = code.replace(/let currentAdminToken = null;\n/, '');

    // 2. Add ADMIN_CHAT_ID configuration
    ...
    // 3. Update normalizeSubscription
...
const normalizeSubscription = (user) => {
    ...
    // 4. Update /api/generate-native-pdf block
    ...
    // 5. Update point deduction in /api/generate-native-pdf
    // Find where reports are pushed and points are deducted
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('admin_')) {
        ...
        if (data.startsWith('admin_user_')) {
            ...
            // admin_user_12345 or admin_user_action_12345
            if (parts.length === 3) {
...
الأيام المتبقية: \"+sub.subscriptionDays+" يوم
...
// Global text message handler for Admin State Machine
bot.on('message', async (msg) => {
    ...
    if (state.step === 'ADDPOINTS_REASON' || state.step === 'REMOVEPOINTS_REASON') {
        ...
        if (sub) {
            ...
            sub.points = sub.balance_points; // sync old field
...
// ==========================================
`;
    // Insert admin logic before API Endpoints
    code = code.replace(/\/\/ API Endpoints/g, adminLogic + '\n// API Endpoints');
