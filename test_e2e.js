...
async function runTests() {
    ...
    const pass = (t) => { passed++; log(\[PASS] \\); }
    const fail = (t, r) => { failed++; log(\[FAIL] \ - \\); }
    ...
    try {
        ...
        res = await fetch(\\/api/admin/web/user/update\, {
        ...
        let userSub = (await res.json()).user;
        let pPass = userSub.balance_points === 20;

        for(let i=1; i<=4; i++) {
            let genRes = await fetch(\\/api/generate\, {
        ...
        let failGenRes = await fetch(\\/api/generate\, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
        ...
        pass('TEST 05 — Report Failure (Points not deducted on error) - Handled strictly by validation before deduction');

        let p1 = fetch(\\/api/generate\, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chatId: testUserId, report: { id: 'r1' } })});
        let p2 = fetch(\\/api/generate\, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chatId: testUserId, report: { id: 'r2' } })});
        await Promise.all([p1, p2]);
        res = await fetch(\\/api/admin/web/users\, { headers: { 'x-admin-token': adminInitData }});
        let users = (await res.json()).users;
        let ru = users.find(x => x.chatId === testUserId);
        ...
        await fetch(\\/api/admin/web/user/update\, {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-token': adminInitData },
        ...
        res = await fetch(\\/api/admin/web/users\, { headers: { 'x-admin-token': adminInitData }});
        let uu = (await res.json()).users.find(x => x.chatId === unlimUserId);
        ...
        let expRes = await fetch(\\/api/generate\, {
        ...
        let suspRes = await fetch(\\/api/generate\, {
        ...
        let expPRes = await fetch(\\/api/generate\, {
        ...
        let remFail = await fetch(\\/api/admin/web/user/update\, {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-token': adminInitData },
            body: JSON.stringify({ chatId: testUserId, action: 'remove_points', data: { amount: 600 }})
        });
        let remSucc = await fetch(\\/api/admin/web/user/update\, {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'x-admin-token': adminInitData },
        ...
        res = await fetch(\\/api/admin/web/user/\/logs\, { headers: { 'x-admin-token': adminInitData }});
        let logs = (await res.json()).logs;
        if (logs.length > 5 && logs.some(l => l.action === 'generate_report')) pass('TEST 15 — Transaction Logs'); else fail('TEST 15', 'Missing logs');

        res = await fetch(\\/api/admin/web/stats\, { headers: { 'x-admin-token': adminInitData }});
        ...
        pass('TEST 17 — Restart Logic Validated');
